...
def rozwiniecie1(imie):
    ...
    while True:
        ...
        draw_obstacles([goal])
        print("\033[1;1HPoruszaj się klawiszami W(gora) A(lewo) S(dol) D(prawo). Naciśnij 'q' aby oposcic gre.")
        ...
        if key == 'w':
            ...
            print(f"{Colors.RED}GRA ZOSTALA ZATRZYMANA{Colors.END}")
            break

        if check_collision(y, x, obstacles):
            clear_screen()
            print(f"{Colors.RED}PRZEGALES. SPROBUJ PONOWNIE.{Colors.END}")
            break

        if (y, x) == (goal[0], goal[1]):
            clear_screen()
            print(f"{Colors.GREEN}UDALO CI SIE!")
            start()
            break
